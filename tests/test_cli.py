...
def test_detect_command_uses_application_result_and_writes_annotation() -> None:
    ...
    try:
        ...
        assert "Detected 1" in stdout.getvalue()
        assert use_case.calls[0][2] == 50
    finally:
        image_path.unlink(missing_ok=True)
        output_path.unlink(missing_ok=True)


def test_legacy_box_function_delegates_to_use_case() -> None:
...
def test_download_command_delegates_complete_workflow_to_create_scan() -> None:
    RUNTIME.mkdir(parents=True, exist_ok=True)
    ...
    assert exit_code == 0
    assert roots == [output_root.resolve()]
    assert use_case.calls[0][1] == 7
    assert "scan.png" in stdout.getvalue()


def test_ingest_command_reports_plugin_outcomes_and_failure_exit_code() -> None:
    result = {
        "total_inserted": 2,
        "logs": [
    ...
    assert exit_code == 1
    assert "two: FAILED" in stdout.getvalue()
    assert use_case.calls[0][1] == (datetime(2026, 8, 27, 6, tzinfo=timezone.utc), now)


def test_predict_command_uses_configured_key_and_formats_predictions() -> None:
    ...
    assert exit_code == 0
    assert use_case.calls[0][1] == "configured-key"
    assert "max elev: 42°" in stdout.getvalue()


def test_commands_translate_expected_errors_to_stderr_and_exit_one() -> None:
    command = PredictCommand(
    ...
    assert exit_code == 1
    assert stderr.getvalue().strip() == "Error: bad prediction"


def test_unified_dispatcher_forwards_remaining_arguments() -> None:
    ...
    assert exit_code == 7
    assert received[0] == BBOX_ARGUMENTS


def test_unified_dispatcher_forwards_subcommand_help() -> None:
    dispatcher = importlib.import_module("sentinel_analysis.interfaces.cli.__main__")
    original = dispatcher.COMMANDS["predict"]
    received = []
